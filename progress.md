...
- [x] フレーム単位分析の導入（Agentic Vision相当の高精度検出）
  ...
  - 最小間隔0.7秒、補完間隔2秒に変更
- [x] 1:39以降が抽出されない問題を修正
- [x] コードベースのカット検出を追加（LLM不要・高速・トークン節約）
- [x] シーン検出をPython（PySceneDetect）に統一、分析はGemini 2.5 Flashのまま
  - utils/sceneDetector.ts: フレーム差分によるカット検出（ContentDetector相当）
  - SceneUploader: 検出モード選択UI（コード / LLM）
  - コードモード: 明確なカット割りのみ、テロップ変化は検出しない
  - extractFramesAtFixedInterval: 動画全長をカバーするよう間隔を自動調整（maxFrames超過時は間隔を広げて全長をカバー）
  - maxFrames 100→200 に増加（3分20秒まで1秒間隔でカバー）
